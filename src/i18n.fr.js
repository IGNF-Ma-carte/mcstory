...
L'outil propose par défaut les Déparement d'outre-mer (DOM) et les Territoires d'outre-mer (TOM), mais vous pouvez aussi importer une liste d'objets sur lesquels vous voulez vous focaliser (via un fichier géographique, GeoJSON, GPX ou KML). Dans ce cas, les objets doivent avoir un attribut *nom* (ou *name*) qui s'affichera dans la liste.
*NB : pour plus de lisibilité, limitez le nombre de zone de focalisation au risque de ne pas pouvoir les afficher correctement.*
`,
